...
public class OrderBookTest extends BaseCase {
    ...
    public Object[][] provideorderBookUrlErrorData(Method method){
        String path = "./src/main/resources/io/cex/test/autotest/interfacecase/cex/orderBook/error";
     ...
     * @param
     **/
    @Test(dataProvider = "provideorderBookUrlErrorData",description = "orderBook异常用例")
    public void testorderBookDataError(Map<?,?> param) throws IOException {
        ...
        Response response = OkHttpClientManager.post(ip+orderBookUrl, jsonbody.toJSONString(),
     ...
     */
    @DataProvider(parallel=true)
    public Object[][] provideorderBookData(Method method){
        String path = "./src/main/resources/io/cex/test/autotest/interfacecase/cex/orderBook/orderBook.json";
        HashMap<String, String>[][] arrymap = (HashMap<String, String>[][]) JsonFileUtil.jsonFileToArry(path);
        return arrymap;
    }
    @Severity(SeverityLevel.CRITICAL)
    @Test(dataProvider = "provideorderBookData",description = "检查返回是否成功")
    public void testorderBookData(Map<?,?> param) throws IOException {
        dataInit();
        JSONObject object = JSON.parseObject(param.get("body").toString());
        JSONObject jsonbody = new JSONObject();
        jsonbody.put("lang",lang);
